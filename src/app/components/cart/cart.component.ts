...
export class CartComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (cartId) {
      this.http.get(`https://backendtravelbazaar-3krb.onrender.com/api/travelbazaar/v1/cart-items/shopping-carts/${cartId}`)
  ...
  removeFromCart(item: any) {
    ...
    this.http.delete(`https://backendtravelbazaar-3krb.onrender.com/api/travelbazaar/v1/cart-items/${cartItemId}`)
      .subscribe(() => {
  ...
  loadShoppingCart() {
    ...
    if (cartId) {
      this.http.get(`https://backendtravelbazaar-3krb.onrender.com/api/travelbazaar/v1/cart-items`)
        .subscribe((response: Object) => {
          this.shoppingCart = response as any[];
        });
    }
